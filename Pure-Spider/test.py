...
url="https://www.zhihu.com/collection/60771406"#指定的URL	
  
  
def download(_url,name):#下载函数  
	if(_url==None):#地址若为None则跳过	 
		pass  
	req = urlopen(_url)#打开链接  
	#print req.getcode()  
	if(req.getcode()!=200):#如果链接不正常，则跳过这个链接	 
		pass  
	else:  
		data=req.read()#否则开始下载到本地  
		with open(name, "wb") as code:
			code.write(data)
			code.close()
  
  
res = urlopen(url)#打开目标地址  
...
for address in s:  
	if(address!=None):	
		pathName="E:\\2666\\"+str(count+1)+".jpg"#设置路径和文件名	
